...
class Model_ConfigFactorRule(BaseModel):
    ...
    factor_id = db.Column(db.ForeignKey(f"{CONFIG_FACTOR}.factor_id", onupdate="CASCADE", ondelete="CASCADE"))
    class_name = db.Column(db.String(100))
    field_name = db.Column(db.String(100), nullable=False)
    comparison_operator_code = db.Column(db.String(10), db.ForeignKey(
        f"{REF_COMPARISON_OPERATOR}.comparison_operator_code", onupdate="CASCADE"), nullable=False)
