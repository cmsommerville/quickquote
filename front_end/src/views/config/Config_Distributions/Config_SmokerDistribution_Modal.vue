<template>
  <app-modal ref="modal" v-bind="$attrs" @close:modal="$emit('close:modal')">
    <template #header>Edit Smoker Status Distribution</template>
...
import { CONFIG_SMOKER_STATUS_DISTRIBUTION__COLUMN_DEFS } from "./config.js";
import { Model_ConfigAttrDistribution } from "@/models/Model_ConfigDistributions.js";

export default {
  name: "Config_SmokerStatusDistribution_Modal",
  ...
  data() {
    return {
      columnDefs: [...CONFIG_SMOKER_STATUS_DISTRIBUTION__COLUMN_DEFS],
    };
  },
  methods: {
    initialize() {},
