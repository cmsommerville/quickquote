      ...
      <div class="flex justify-center flex-col my-12">
        <div class="h-72 w-full mx-auto relative">
          ...
          <app-new-edit-delete-button
            :disabled_edit="!_selection.age_distribution_set_id"
            :disabled_delete="!_selection.age_distribution_set_id"
            @fab:new="newHandler"
            @fab:edit="editHandler"
            @fab:delete="deleteHandler"
          />
          <config-age-distribution-modal
            class="hidden"
            ref="modal"
            v-model="_selection"
...
import ConfigAgeDistributionModal from "./Config_AgeDistribution_Modal.vue";
import { AgGridVue } from "ag-grid-vue3";
import { CONFIG_AGE_DISTRIBUTION_LIST__COLUMN_DEFS } from "./config.js";
import { Model_ConfigAgeDistributionSet } from "@/models/Model_ConfigDistributions.js";
...
export default {
  ...
  components: {
    ...
    ConfigAgeDistributionModal,
  },
  async mounted() {
    this.loaded = false;
    const res = await axios.get(`/qry-config/age-distribution-sets`);
  ...
  data() {
    return {
      ...
      distributions: [],
      columnDefs: [...CONFIG_AGE_DISTRIBUTION_LIST__COLUMN_DEFS],
      _selection: {},
    };
  },
  methods: {
    addDistributionHandler(data) {
      this.distributions = [
        ...this.distributions.filter((dist) => {
          return (
            dist.age_distribution_set_id == null ||
            dist.age_distribution_set_id !== data.age_distribution_set_id
          );
        }),
    ...
    async deleteHandler() {
      if (this._selection.age_distribution_set_id) {
        await axios.delete(
          `/config/age-distribution-set/${this._selection.age_distribution_set_id}`
        );
    ...
    modelSetter(data) {
      const dist = new Model_ConfigAgeDistributionSet(
        data.age_distribution_set_id,
        data.age_distribution_set_label,
  ...
  computed: {
    ...
    rowData() {
      return this.distributions.map((item) => {
        return {
          ...item,
          age_count: item.age_distribution.length ?? 0,
          average_age: (
            item.age_distribution.reduce((prev, curr) => {
              return prev + +curr.weight * +curr.age;
            }, 0) /
            item.age_distribution.reduce((prev, curr) => {
