    ...
    <div class="my-12" v-if="loaded">
      <div class="flex justify-center flex-col my-12">
        <div class="h-72 w-full mx-auto relative">
          ...
          />
          <config-smoker-status-distribution-modal
            class="hidden"
            ref="modal"
            v-model="_selection"
...
import ConfigSmokerStatusDistributionModal from "./Config_SmokerDistribution_Modal.vue";
import { AgGridVue } from "ag-grid-vue3";
import { CONFIG_SMOKER_STATUS_DISTRIBUTION_LIST__COLUMN_DEFS } from "./config.js";
...
export default {
  ...
  components: {
    ...
    ConfigSmokerStatusDistributionModal,
  },
  props: {},
  async mounted() {
    this.loaded = false;
  ...
  data() {
    return {
      loaded: false,
      show_fab: false,
      title: "Smoker Status Distributions",
      subtitle: "Create a new distribution or edit an existing one!",
      active_stage: "smoker_status",
      _stages: [
        {
        ...
        {
          label: "Smoker Distributions",
          id: "smoker_status",
          disabled: true,
        },
      ],
      distributions: [],
      columnDefs: [...CONFIG_SMOKER_STATUS_DISTRIBUTION_LIST__COLUMN_DEFS],
      _selection: {},
    };
  },
  methods: {
    ...
    newHandler() {
      this._selection = {
        attr_distribution: [{ attr_value: "N" }, { attr_value: "T" }],
      };
